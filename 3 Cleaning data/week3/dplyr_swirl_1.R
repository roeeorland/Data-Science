...
2: Getting and Cleaning Data Tidying Data with tidyr
...
| In this lesson, you'll learn how to manipulate data using dplyr. dplyr is a fast and powerful R package written by Hadley Wickham and Romain Francois that provides
| a consistent and concise grammar for manipulating tabular data.

...

  |                                                                                         |=                                                                                |   2%
| One unique aspect of dplyr is that the same set of tools allow you to work with tabular data from a variety of sources, including data frames, data tables,
| databases and multidimensional arrays. In this lesson, we'll focus on data frames, but everything you learn will apply equally to other formats.

...

  |                                                                                         |===                                                                              |   3%
| As you may know, "CRAN is a network of ftp and web servers around the world that store identical, up-to-date, versions of code and documentation for R"
| (http://cran.rstudio.com/). RStudio maintains one of these so-called 'CRAN mirrors' and they generously make their download logs publicly available
| (http://cran-logs.rstudio.com/). We'll be working with the log from July 8, 2014, which contains information on roughly 225,000 package downloads.

...

  |                                                                                         |====                                                                             |   5%
| I've created a variable called path2csv, which contains the full file path to the dataset. Call read.csv() with two arguments, path2csv and stringsAsFactors =
| FALSE, and save the result in a new variable called mydf. Check ?read.csv if you need help.
...
| The dplyr package was automatically installed (if necessary) and loaded at the beginning of this lesson. Normally, this is something you would have to do on your
...
| The first step of working with data in dplyr is to load the data into what the package authors call a 'data frame tbl' or 'tbl_df'. Use the following code to create
| a new tbl_df called cran:
| 
| cran <- tbl_df(mydf).
...
| From ?tbl_df, "The main advantage to using a tbl_df over a regular data frame is the printing." Let's see what is meant by this. Type cran to print our tbl_df to
| the console.

> 
> cran
# A tibble: 225,468 ª 11
       X       date     time    size r_version r_arch      r_os      package version country ip_id
...
| just shows us the first 10 rows of data and only as many columns as fit neatly in our console. At the bottom, we see the names and classes for any variables that
...
| Instead of specifying the columns we want to keep, we can also specify the columns we want to throw away. To see how this works, do select(cran, -time) to omit the
| time column.

> select(cran,-time)
# A tibble: 225,468 ª 10
...
| Oops! That gaves us a vector of numbers from -5 through 20, which is not what we want. Instead, we want to negate the entire sequence of numbers from 5 through 20,
| so that we get -5, -6, -7, ... , -18, -19, -20. Try the same thing, except surround 5:20 with parentheses so that R knows we want it to first come up with the
...
> select(cran,-(x:size))
# A tibble: 225,468 ª 8
       X r_version r_arch      r_os      package version country ip_id
...
# ... with 225,458 more rows
Warning message:
In x:size : numerical expression has 9 elements: only the first used
...
| The == operator asks whether the thing on the left is equal to the thing on the right. If yes, then it returns TRUE. If no, then FALSE. In this case, package is an
| entire vector (column) of values, so package == "swirl" returns a vector of TRUEs and FALSEs. filter() then returns only the rows of cran corresponding to the
| TRUEs.
...
| The conditions passed to filter() can make use of any of the standard comparison operators. Pull up the relevant documentation with ?Comparison (that's an uppercase
| C).
...
| Edit your previous call to filter() to instead return rows corresponding to users in "IN" (India) running an R version that is less than or equal to "3.0.2". The up
| arrow on your keyboard may come in handy here. Don't forget your double quotes!

> filter(cran, r_version<="3.1.1",country=="IN")
# A tibble: 15,664 ª 11
       X       date     time     size r_version r_arch      r_os   package  version country ip_id
   <int>      <chr>    <chr>    <int>     <chr>  <chr>     <chr>     <chr>    <chr>   <chr> <int>
1    348 2014-07-08 00:44:04 10218907     3.0.0 x86_64   mingw32        BH 1.54.0-2      IN   112
2   4816 2014-07-08 01:04:27  3382454     3.1.0 x86_64   mingw32    Matrix    1.1-4      IN   654
3   5240 2014-07-08 01:36:38    17561     3.1.0 x86_64   mingw32   memoise    0.2.1      IN   712
4   5241 2014-07-08 01:36:43   667053     3.1.0 x86_64   mingw32 lubridate    1.3.3      IN   712
5   5680 2014-07-08 01:45:14  2674377     3.1.0 x86_64 linux-gnu   ggplot2    1.0.0      IN   712
6   6103 2014-07-08 01:04:03 15366176     3.1.0 x86_64   mingw32 RQuantLib   0.3.12      IN   654
7   6135 2014-07-08 01:06:35   770514     3.1.0 x86_64   mingw32    hexbin   1.26.3      IN   112
8   8422 2014-07-08 01:04:22     9390     3.1.0 x86_64   mingw32       pwr    1.1.1      IN   978
9   9470 2014-07-08 01:04:22  1045238     3.1.0   i386   mingw32      MASS   7.3-33      IN   654
10  9534 2014-07-08 01:04:33  1763711     3.1.0   i386   mingw32      mgcv    1.8-1      IN   654
# ... with 15,654 more rows
...
| Our last two calls to filter() requested all rows for which some condition AND another condition were TRUE. We can also request rows for which EITHER one condition
| OR another condition are TRUE. For example, filter(cran, country == "US" | country == "IN") will gives us all rows for which the country variable equals either "US"
| or "IN". Give it a go.

> filter(cran, country=="US" | country=="IN")
# A tibble: 95,283 ª 11
       X       date     time    size r_version r_arch      r_os      package version country ip_id
   <int>      <chr>    <chr>   <int>     <chr>  <chr>     <chr>        <chr>   <chr>   <chr> <int>
1      1 2014-07-08 00:54:41   80589     3.1.0 x86_64   mingw32    htmltools   0.2.4      US     1
2      2 2014-07-08 00:59:53  321767     3.1.0 x86_64   mingw32      tseries 0.10-32      US     2
...
| Now, use filter() to fetch all rows for which size is strictly greater than (>) 100500 (no quotes, since size is numeric) AND r_os equals "linux-gnu". Hint: You are
| passing three arguments to filter(): the name of the dataset, the first condition, and the second condition.

> filter(cran, size>100500 & r_os == "linux-gnu")
# A tibble: 33,683 ª 11
       X       date     time    size r_version r_arch      r_os  package version country ip_id
   <int>      <chr>    <chr>   <int>     <chr>  <chr>     <chr>    <chr>   <chr>   <chr> <int>
...
| This is a tricky one. filter(cran, size > 100500, r_os == "linux-gnu") will give us all rows for which size is strictly greater than 100500 and r_os is "linux-gnu".
...
| Finally, we want to get only the rows for which the r_version is not missing. R represents missing values with NA and these missing values can be detected using the
| is.na() function.
...
| Now, put an exclamation point (!) before is.na() to change all of the TRUEs to FALSEs and all of the FALSEs to TRUEs, thus telling us what is NOT NA: !is.na(c(3, 5,
| NA, 10)).
...
| Okay, ready to put all of this together? Use filter() to return all rows of cran for which r_version is NOT NA. Hint: You will need to use !is.na() as part of your
| second argument to filter().

> filter(cran,!is.na(r_version))
# A tibble: 207,205 ª 11
       X       date     time    size r_version r_arch      r_os      package version country ip_id
   <int>      <chr>    <chr>   <int>     <chr>  <chr>     <chr>        <chr>   <chr>   <chr> <int>
1      1 2014-07-08 00:54:41   80589     3.1.0 x86_64   mingw32    htmltools   0.2.4      US     1
2      2 2014-07-08 00:59:53  321767     3.1.0 x86_64   mingw32      tseries 0.10-32      US     2
...
| Now, to order the ROWS of cran2 so that ip_id is in ascending order (from small to large), type arrange(cran2, ip_id). You may want to make your console wide enough
| so that you can see ip_id, which is the last column.
...
| To illustrate the next major function in dplyr, let's take another subset of our original data. Use select() to grab 3 columns from cran -- ip_id, package, and size
...
| One very nice feature of mutate() is that you can use the value computed for your second column (size_mb) to create a third column, all in the same line of code. To
| see this in action, repeat the exact same command as above, except add a third argument creating a column that is named size_gb and equal to size_mb / 2^10.
...
4      3        Hmisc  605104
5      4       digest   78825
6      3 randomForest   76681
7      3         plyr  392754
8      5      whisker   27216
9      6         Rcpp    4928
10     7     hflights 2205029
# ... with 225,458 more rows

| Nice try, but that's not exactly what I was hoping for. Try again. Or, type info() for more options.

| mutate(cran3, correct_size = size + 1000) will create one new column that is equal to the original sizes plus 1000.
...
> summarize(cran,yahoo=mean(size))
# A tibble: 1 ª 1
     yahoo
     <dbl>
1 844086.5
...
| summarize(cran, avg_bytes = mean(size)) will give us the mean size and label the result 'avg_bytes'.
...
What is your assignment token? Gg3lVtPSTFRXtVhp
